...
__ssh_ident_cli_flag_verbose=256


define_ssh_ident_bash_funcs () {
	# SSH_IDENT_SSH_SCRIPT must be global to be available when the functions are executed
	SSH_IDENT_SSH_SCRIPT=`which ssh_ident_ssh`
	export GIT_SSH=$SSH_IDENT_SSH_SCRIPT
	rsync() { BINARY_SSH=ssh command rsync -e $SSH_IDENT_SSH_SCRIPT "$@"; }
	sftp()  { BINARY_SSH=ssh command sftp -S $SSH_IDENT_SSH_SCRIPT "$@"; }
	scp()   { BINARY_SSH=ssh command scp -S $SSH_IDENT_SSH_SCRIPT "$@"; }
	ssh()   { BINARY_SSH=ssh command $SSH_IDENT_SSH_SCRIPT "$@"; }
}
...
ssh_ident() {
	...
	# If cli_actionurn code does not match, show output from ssh_ident_cli
